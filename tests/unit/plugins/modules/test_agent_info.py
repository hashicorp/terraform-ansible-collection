...
"""Unit tests for the agent_info module."""

from unittest.mock import Mock, patch

from ansible_collections.hashicorp.terraform.plugins.modules.agent_info import main

MODULE_PATH = "ansible_collections.hashicorp.terraform.plugins.modules.agent_info"
...
@patch(f"{MODULE_PATH}.get_agent")
def test_by_id_success(mock_get, mock_module_class):
    mock_module, mock_adapter = _mock_module({"agent_id": "agent-1"})
    mock_module_class.return_value = mock_module
    mock_get.return_value = {"id": "agent-1", "name": "my-agent", "status": "idle"}

    main()

    mock_get.assert_called_once_with(mock_adapter, "agent-1")
...
def test_by_id_not_found_fails(mock_get, mock_module_class):
    mock_module = _mock_module({"agent_id": "agent-x"})[0]
    mock_module_class.return_value = mock_module
